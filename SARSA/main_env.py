...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.algorithm = algorithm  # whether this is a windy environment
    ...
    def step(self, action):
        ...
        if self.algorithm == 'dqn':
            old_x, old_y = self._state_to_xy(self.agent_state)
        else:
        ...
        if self.algorithm == 'dqn':
            self.agent_state = self._xy_to_state(new_x, new_y)
            env_states = self.get_map_state()
            return env_states, self.reward, done, info
        else:
    ...
    def reset(self):
        if self.algorithm == 'dqn':
            obs_states = self.get_map_state()
            self.agent_state = self._xy_to_state(self.start)  # the start
            return obs_states
        else:
    ...
    def get_map_state(self):
        # map
        maze_env = np.zeros((self.n_width, self.n_height))
    ...
    def render(self, mode='human', close=False):
        ...
        if self.algorithm == 'dqn':
            x, y = self._state_to_xy(self.agent_state)
        else:
...
def maze_grid(algorithm):
    ...
    env.start = (2, 9)
    env.ends = [(5, 4)]
    env.types = [(4, 2, 1), (4, 3, 1), (4, 4, 1), (4, 5, 1), (4, 6, 1),
...
def main(algorithm):
    ...
    if algorithm == 'sarsa':
    ...
    elif algorithm == 'sarsa_n':
        agent = sarsa_nstep.SarsaLambdaAgent(env)
        print("Learning...")
        ...
        agent = DQN_agent.QAgent(env,
                                 memory_capacity=10000,  # experience memory
                                 hidden_dim=10)
        env.reset()
        print("Learning...")
        agent.learning(max_episodes=3,
                       batch_size=64,
                       gamma=0.9,
                       min_epsilon=0.1)
...
if __name__ == "__main__":
    algorithm = 'sarsa'  # sarsa, sarsa_n, dqn
    # the dqn need modify the reward
    main(algorithm)
