...
class DQN(object):
    ...
    def train(self, batch_size=64, lr=1, factor=0.95):
        ...
        # (s, a, next_s, reward, done)
        s_batch = np.array([replay[0] for replay in replay_batch])
        action = np.array([replay[1] for replay in replay_batch])
        next_s_batch = np.array([replay[2] for replay in replay_batch])
        done = np.array([replay[4] for replay in replay_batch])
        ...
        Q[range(len(s_batch)), action] = reward + (factor * max_Q)*(~done)
...
for i in range(episodes):
    ...
    while True:
        ...
        if done:
            ...
            print('episode:', i, 'score:', score, 'max:', max(score_list), 'time:', time.time()-start_time)
            break
